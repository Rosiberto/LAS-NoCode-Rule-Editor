...
function getHtml(title, fields) {	
    ...
    <div>
      ...
      <div class="box">
        ...
        <input type="text" placeholder="Enter sum name." df-sumname>
...
function updateNodeHtml(editor, nodeId) {
 ...
 // Pegamos o HTML base
  ...
  const sumValue = node.data.sumname || '';
  const eventValue = node.data.eventname || '';
  
  // Atualiza o primeiro input (df-sumname)
  html = html.replace(
    /<input([^>]*)df-sumname([^>]*)>/,
    `<input$1df-sumname$2 value="${sumValue}" onchange="window.updateNode('${nodeId}', 'sumname', this.value')">`
  );
